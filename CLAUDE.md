...
- Layout compacts saved node sizes before computing graph geometry, then shrinks group rectangles to the laid-out node contents plus padding while reserving a visible title/header band above member nodes.
...
- The GUI exposes group CRUD controls: start drag-based group creation from the canvas, rename or delete a single group from the canvas, and clear all groups from the toolbar. The toolbar no longer has a dedicated `+ Group` button.
- Linked ungrouped nodes are positioned by local dataflow layers before vertical placement, instead of being ordered only by original Y position. Group-bridge placement is reserved for true group-to-group bridge nodes; source-only chains downstream of loader groups stay in normal dataflow layers instead of stacking in one bridge slot. Ungrouped bridge nodes that connect freely movable groups are placed in the gaps between those group columns, aligned to their source/target ports, and given enough compact spacing to avoid dropping below the destination group. Bridge-derived group dependencies are skipped for pinned or partially pinned group geometry, but fixed group surfaces still anchor incident bridge nodes so decode/adapter nodes do not drift beyond the movable source group. Each bridge node scores its x candidates (the fixed-incident gap and the layer slot) by resulting cable length and keeps the shortest; the fixed-incident gap still wins when it is free, but the layer slot wins when that gap is blocked by an unrelated tall non-incident group, which previously let the down-only Y resolver shove the bridge far below that group. Bridge nodes are placed before the regular ungrouped columns and then re-placed once those columns exist, passing the regular node rectangles as obstacles, so a small bridge is not left buried under an oversized terminal node positioned afterwards.
- Internal grouped node ordering uses forward/backward barycenter sweeps against adjacent layer order to reduce crossings. Long internal layer chains can wrap into additional rows so compact groups use vertical space before becoming wide strips.
