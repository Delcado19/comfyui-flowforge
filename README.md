...
2. **Crossing minimisation** — nodes within each layer are reordered using the *barycenter heuristic*: each node's score is the average position of its neighbours in the adjacent layer. Two passes are run (forward then backward) to reduce edge crossings.
